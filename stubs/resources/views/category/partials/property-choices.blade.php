@foreach($category->properties as $property)
    @if($property->choices->isNotEmpty())
        <div class='d-grid gap-1 mb-5'>
            ...
            @includeWhen($property->isCheckbox() && $property->name !== 'Χρώμα', 'category.partials.filter-checkbox')
            @includeWhen($property->isCheckbox() && $property->name === 'Χρώμα', 'category.partials.filter-color')
            @includeWhen($property->isRadio(), 'category.partials.filter-radio')
        </div>
    @endif
@endforeach
