    ...
    <div class='d-flex flex-column filters'>
        <div class='d-flex mb-3'>
            ...
            @if ($filters['m']->isNotEmpty())
                <a href='{{ category_route($category, NULL, $filters['c'], $filters['min_price'], $filters['max_price']) }}' class='small ms-3'>{{ __('Clear') }}</a>
            @endif
        </div>
        @foreach($manufacturers as $manufacturer)
            <div class="form-check">
                <input type="checkbox"
                       ...
                       onchange="location.href = '{{ category_route($category, $filters['m']->toggle($manufacturer), $filters['c'], $filters['min_price'], $filters['max_price']) }}'">
