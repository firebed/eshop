<div class="filters offcanvas offcanvas-start" tabindex="-1" id="filters" aria-labelledby="filters">
    <div class="offcanvas-header">
        <div class="d-flex align-items-baseline gap-3">
            ...
            @if ($selectedManufacturers->isNotEmpty() || request()->filled('min_price') || request()->filled('max_price'))
                <a href="{{ route('customer.products.search.index', app()->getLocale()) }}" class="text-hover-underline">
    ...
    <div class="offcanvas-body">
        @includeWhen($categories->isNotEmpty(), 'eshop::customer.product-search.partials.categories')
        @includeWhen($manufacturers->isNotEmpty(), 'eshop::customer.product-search.partials.manufacturers')
        @includeWhen(!empty($priceRanges), 'eshop::customer.product-search.partials.prices')
    </div>
