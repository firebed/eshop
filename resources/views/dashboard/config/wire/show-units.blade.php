    ...
    <x-bs::card.body>
        ...
        <x-bs::table>
            ...
            @forelse($units as $unit)
                <tr wire:key="row-{{ $unit->id }}" wire:loading.class.delay="opacity-50" wire:target="save, edit, delete">
                    <td class="align-middle">
                        <x-bs::input.checkbox wire:model="selected" value="{{ $unit->id }}" id="cb-{{ $unit->id }}"/>
                    </td>
                    <td>{{ $unit->name }}</td>
                    <td class="text-end">
                        <a href="#" wire:click.prevent="edit({{ $unit->id }})"><em class="far fa-edit"></em></a>
                    </td>
                </tr>
            @empty
            ...
            <caption>
                <x-eshop::wire-pagination :paginator="$units"/>
            </caption>
        </x-bs::table>
    </x-bs::card.body>

    <form wire:submit.prevent="save">
        @include('eshop::dashboard.config.partials.unit-modal')
    </form>
        ...
        <x-bs::modal wire:model.defer="showConfirmDelete">
            <x-bs::modal.body>
                <div class="d-grid gap-3 text-center">
                    ...
                    <div class="text-secondary">{{ __("Are you sure you want to delete the selected units? This action cannot be undone.") }}</div>
                    <div>
