<x-bs::card class="h-100">
    <x-bs::card.body>
        ...
        <x-bs::table>
            ...
            <tbody>
            @forelse($vats as $vat)
                <tr wire:key="row-{{ $vat->id }}" wire:loading.class.delay="opacity-50" wire:target="save, edit, delete">
                    <td class="align-middle">
                        <x-bs::input.checkbox wire:model="selected" value="{{ $vat->id }}" id="cb-{{ $vat->id }}"/>
                    </td>
                    <td>{{ $vat->name }}</td>
                    <td>{{ format_percent($vat->regime) }}</td>
                    <td class="text-end">
                        <a href="#" wire:click.prevent="edit({{ $vat->id }})"><em class="far fa-edit"></em></a>
                    </td>
                </tr>
            @empty
            ...
            <caption>
                <x-eshop::wire-pagination :paginator="$vats"/>
            </caption>
        </x-bs::table>
    </x-bs::card.body>

    <form wire:submit.prevent="save">
        @include('eshop::dashboard.config.partials.vat-modal')
    </form>
        ...
        <x-bs::modal wire:model.defer="showConfirmDelete">
            <x-bs::modal.body>
                <div class="d-grid gap-3 text-center">
                    ...
                    <div class="text-secondary">{{ __("Are you sure you want to delete the selected vats? This action cannot be undone.") }}</div>
                    <div>
