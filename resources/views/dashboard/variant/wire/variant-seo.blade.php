<div x-data="{ open: {{ $errors->has('seo.*') || $errors->has('slug') ? 'true' : 'false' }} }"
     x-on:variant-options-updated.window="$wire.setOptions($event.detail)"
     class="d-grid gap-2"
>
    ...
    <div class="d-grid" style="max-width: 652px">
        ...
        <div style="color: #4D5156">{{ $description ?: $productDescription }}</div>
    </div>
    ...
    <div x-cloak x-show="open" x-transition>
        <div class="d-grid gap-3">
            ...
            <x-bs::input.group for="seo-description" label="{{ __('Description') }}">
                <x-bs::input.textarea wire:model="description" name="seo[description]" error="seo.description" id="seo-description" rows="5"/>
            </x-bs::input.group>
        </div>
    </div>
</div>