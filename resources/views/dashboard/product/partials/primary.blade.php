<x-bs::card>
    <x-bs::card.body class="d-grid gap-3">
        ...
        <x-bs::input.group for="description" label="{{ __('Description') }}">
            <x-eshop::rich-text x-effect="$dispatch('product-description-updated', text.trim())" :value="old('description', $product->description ?? '')" name="description" error="description" id="description" rows="8" plugins="lists" toolbar="fontselect | bold italic underline | forecolor | bullist numlist | removeformat"/>
        </x-bs::input.group>
    </x-bs::card.body>
</x-bs::card>
